...
const CAMPAIGNS = [
  ...
  },
  {
    ...
    campaignGoal: "retargeting" as const,
...
async function seed() {
  ...
  for (const def of CAMPAIGNS) {
    ...
    console.log(`   Generating ${def.adsToGenerate} ads...\n`);
    ...
    for (let i = 0; i < def.adsToGenerate; i++) {
      try {
        ...
        const status = result.isPublishable ? "✅" : "⚠️ ";
        const ratchet = result.qualityRatchetApplied ? " 🔺 RATCHET" : "";
        console.log(
          `   Ad ${String(i + 1).padStart(2, "0")}/${def.adsToGenerate}: score ${result.bestScore.toFixed(2)} | ${result.totalIterations} iter | ${status}${ratchet}`
        );
    ...
    console.log(
      `\n   📊 Results: ${campaignApproved}/${def.adsToGenerate} approved | avg: ${avg} | best: ${best}`
    );
  }
  ...
  console.log(
    `Total approved:   ${totalApproved} (${((totalApproved / totalGenerated) * 100).toFixed(0)}%)`
  );
  console.log("\nOpen the app → Dashboard to see your 3 campaigns.");
  console.log(
    "Go to Performance Tracker inside any campaign to see quality trends.\n"
  );
}
