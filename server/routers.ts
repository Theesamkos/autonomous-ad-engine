...
// ─── In-memory campaign cache (latency optimization) ────────────────────────
...
async function getCampaignCached(campaignId: number) {
  ...
  campaignCache.set(campaignId, { data: fresh, expiresAt: Date.now() + CACHE_TTL_MS });
  return fresh;
}
...
Rewrite this ad for the target persona. Keep the core offer but change language, hooks, and framing.`;
    ...
    // ─── Variety Matrix ────────────────────────────────────────────────────────────────────────────────
    getVarietyMatrix: protectedProcedure.query(() => {
      return {
        tones: ["Urgent", "Empathetic", "Aspirational", "Humorous", "Authoritative", "Conversational", "Provocative", "Inspirational"],
        formats: ["Problem→Solution", "Story", "Social Proof", "Question Hook", "Statistic Lead", "Before/After"],
        hooks: ["Fear of Missing Out", "Curiosity Gap", "Social Proof", "Pain Point", "Dream Outcome"],
        audiences: ["Anxious Parents", "Motivated Students", "Comparison Shoppers", "Budget-Conscious Families", "Last-Minute Preppers"],
      };
    }),
    ...
    expandPrompt: protectedProcedure.input(z.object({
      ...
      const response = await invokeLLM({
        messages: [
          { role: "system", content: `You are a world-class creative director specializing in performance marketing. Given a vague advertising prompt, expand it into 8 distinct creative angles. Each angle must have a completely different tone, format, emotional hook, and audience framing. Return ONLY valid JSON.` },
          { role: "user", content: `Campaign: ${campaign.name}\nProduct: ${campaign.product}\nAudience: ${campaign.audienceSegment}\nVague prompt: "${input.vaguePrompt}"\n\nExpand this into 8 distinct creative angles. Return JSON: { "angles": [ { "angleName": string, "tone": string, "format": string, "emotionalHook": string, "audienceFraming": string, "exampleHeadline": string, "exampleCopy": string } ] }` },
        ],
        response_format: { type: "json_schema", json_schema: { name: "creative_angles", strict: true, schema: { type: "object", properties: { angles: { type: "array", items: { type: "object", properties: { angleName: { type: "string" }, tone: { type: "string" }, format: { type: "string" }, emotionalHook: { type: "string" }, audienceFraming: { type: "string" }, exampleHeadline: { type: "string" }, exampleCopy: { type: "string" } }, required: ["angleName", "tone", "format", "emotionalHook", "audienceFraming", "exampleHeadline", "exampleCopy"], additionalProperties: false } } }, required: ["angles"], additionalProperties: false } } },
      });
      try {
        const parsed = JSON.parse(response.choices[0].message.content as string) as { angles: Array<{ angleName: string; tone: string; format: string; emotionalHook: string; audienceFraming: string; exampleHeadline: string; exampleCopy: string }> };
        return { angles: parsed.angles ?? [] };
      } catch {
        return { angles: [] };
      }
    }),
